    ...
    <div>
      <div className="mx-[5%] 2xl:mx-[15%] mt-10">
        ...
        <div className="mt-10 sm:mt-20 pb-28">
          <h2 className="text-xl sm:text-3xl text-black/80 font-semibold">
            Know more about{" "}
            <span className="text-emerald-500">{project?.name}</span>
          </h2>
          <p className="mt-5 text-sm md:text-base text-black/70 text-justify">
            {project?.des}
          </p>
          <p className="mt-2 hidden lg:block sm:text-base text-black/70 text-justify">
            {project?.moreDetails}
          </p>
          <div className="flex justify-center">
            <div className="flex flex-col-reverse sm:flex-row items-center sm:items-start md:items-center gap-x-5 mt-14 2xl:justify-start">
              <div className="w-full xl:w-[60%] mt-10 sm:mt-0">
                <h3 className="text-2xl text-black/50 font-semibold">
                  Projects Top features :
                </h3>
                <ul className="list-disc ml-5 mt-5">
                  {project?.KeyPoint?.map((point) => (
                    <li className="text-xs md:text-sm my-2 text-black/80">
                      {point}
                    </li>
                  ))}
                </ul>
          ...
          <h3 className="text-xl mt-20 text-black/50 font-semibold">
            Some UI & Functionality's :
          </h3>
            ...
            {project?.imgDes?.map((content) => (
              <div className="mt-10 flex flex-col md:flex-row items-center border rounded-[10px] w-full lg:w-[900px] ui-box p-5 md:p-0">
                ...
                <div className="px-2 sm:px-10 mt-5 md:mt-0 ">
                  <p className="text-sm sm:text-xl md:text-sm font-semibold text-black/80">
                    {content?.featureTitle}
                  </p>
                  <p className="text-xs sm:text-base text-justify mt-5 text-black/70">
                    {content?.featureDes}
                  </p>
                </div>
              </div>
